...
- references/x-api-groups.md: group overview counts and quick navigation
- Updates curated group reference files (e.g. references/users.md) by inserting an autogenerated section
  containing endpoints + docs for that `x-api/<group>` family.
...
Defaults:
  ...
  --out-dir  /Users/jarrod/x-api-skill/references
...
AUTO_BEGIN = "<!-- BEGIN AUTOGENERATED X API DOCS -->"
AUTO_END = "<!-- END AUTOGENERATED X API DOCS -->"
...
def parse_llms(text: str) -> List[LlmsItem]:
    ...
    for line in text.splitlines():
        ...
        if not m:
            continue
        items.append(LlmsItem(title=m.group(1), url=m.group(2)))
    return items
...
def _render_group_autogen(
    *,
    ...
    lines.append("")
    lines.append(f"_Autogenerated from `{llms_url}` for `x-api/{group}`._")
    lines.append("")
    lines.append(f"- Docs pages: {len(group_items)}")
    lines.append(f"- OpenAPI endpoint pages detected: {len(open_items)}")
    lines.append(f"- Non-OpenAPI docs pages: {len(non_items)}")
    lines.append("")

    if open_items:
        lines.append("## Endpoints (Autogenerated)")
        lines.append("")
    ...
    if non_items:
        lines.append("## Docs (Autogenerated)")
        lines.append("")
        for it in non_items:
...
def _upsert_autogen_section(path: str, autogen: str) -> None:
    exists = os.path.exists(path)
    if exists:
        with open(path, "r", encoding="utf-8") as f:
            cur = f.read()
    else:
        # New file: create a minimal curated header so the top reads well.
        title = os.path.basename(path).rsplit(".", 1)[0].replace("-", " ").title()
        cur = (
            f"# {title}\n\n"
    ...
    if AUTO_BEGIN in cur and AUTO_END in cur:
        pre_raw = cur.split(AUTO_BEGIN, 1)[0]
        if pre_raw.strip() == "":
            title = os.path.basename(path).rsplit(".", 1)[0].replace("-", " ").title()
            pre = (
                f"# {title}\n\n"
            ...
            pre = pre_raw.rstrip() + "\n\n"
        post = cur.split(AUTO_END, 1)[1].lstrip()
        out = pre + autogen + post
    else:
        # Append autogen section at end for curated files.
        out = cur.rstrip() + "\n\n" + autogen
...
def _group_to_target_files(group: str) -> List[str]:
    base = "/Users/jarrod/x-api-skill/references"
...
def main(argv: List[str]) -> int:
    ...
    ap.add_argument("--out-dir", default="/Users/jarrod/x-api-skill/references")
    ...
    with open(groups_path, "w", encoding="utf-8") as f:
        f.write("# X API Docs Coverage Map\n\n")
        f.write(f"Generated from `{args.llms_url}`.\n\n")
        f.write("## Groups\n\n")
        for group, count in group_counts.most_common():
            f.write(f"- `{group}`: {count} docs pages\n")
        f.write("\n## Group Reference Files\n\n")
        f.write("This skill keeps one curated reference file per feature area, with an autogenerated section appended or updated between markers.\n\n")
        for group, count in group_counts.most_common():
            targets = _group_to_target_files(group)
            for t in targets:
                f.write(f"- `{group}` -> `{t}`\n")
        f.write("\n")
        f.write("## OpenAPI Coverage\n\n")
        f.write(f"- OpenAPI endpoint pages detected: {len(openapi_by_url)}\n")
        f.write(f"- Non-OpenAPI docs pages: {len(x_api_items) - len(openapi_by_url)}\n")
        f.write(f"- Build time: {elapsed:.1f}s\n")
        if errors:
            f.write("\n## Fetch Errors\n\n")
            f.write("Some pages could not be fetched or parsed within the configured limits.\n\n")
            for url, err in sorted(errors)[:50]:
                f.write(f"- `{url}`: `{err}`\n")
            if len(errors) > 50:
    ...
    with open(catalog_path, "w", encoding="utf-8") as f:
        ...
        f.write("```bash\n")
        f.write("rg -n \"/2/|OAuth2UserToken|BearerToken|tweet.write|dm.write|media\" /Users/jarrod/x-api-skill/references/x-api-catalog.md\n")
        f.write("```\n\n")
    ...
    # Upsert autogenerated sections into curated files.
    for group, items_in_group in grouped.items():
        group_items = sorted(items_in_group, key=lambda it: it.url)
        autogen = _render_group_autogen(
            group=group,
            llms_url=args.llms_url,
            group_items=group_items,
            openapi_by_url=openapi_by_url,
        )
        for target in _group_to_target_files(group):
            _upsert_autogen_section(target, autogen)
